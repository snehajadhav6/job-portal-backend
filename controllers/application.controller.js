...
const applyForJob = async (req, res) => {
  try {
    ...
    let resume_url = req.file.path || req.file.secure_url || req.file.url;

    if (!resume_url) {
      return res.status(400).json({ message: "Cloudinary upload securely failed mapping URL", file: req.file });
    }
